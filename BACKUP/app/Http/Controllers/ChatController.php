...
class ChatController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->rasa_url = 'http://68.183.229.24:5005/webhooks/rest/webhook';
        $this->rasa_ip = '68.183.229.24';
    }
    
    public function index()
    {
        app('session')->put('tes', 'blah');
    ...
    public function chat(Request $request) {
        ...
        $toDatabase = $this->processToDatabase($chats['intent'], $chats['entities'], $chats['response'], $request->input('query'));
        
        $resp = json_encode(['response' => $chats['response']]);
        
        if($toDatabase['error'] == 1) $resp = json_encode(['response' => $chats['response']]);
        
        echo $resp;
        
    }
    
    public function processToDatabase($intent, $entities, $bot_answer, $text) {
        switch ($intent) {
            case 'nama':
            ...
            case 'konfirmasi':
                ...
                break;
            default:
                if($_SESSION['sender']) $user_id = $_SESSION['sender']; Message::create(compact('user_id', 'bot_answer', 'text', 'intent', 'entities'));
